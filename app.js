...
db.tasks=db.tasks||[];db.study=db.study||[];db.wellness=db.wellness||[];db.travel=db.travel||[];db.ideas=db.ideas||[];db.travelStays=db.travelStays||[];db.travelDocs=db.travelDocs||[];db.emergencyContacts=db.emergencyContacts||[];db.travelFinance=db.travelFinance||[];db.healthProviders=db.healthProviders||[];db.healthContacts=db.healthContacts||[];db.patientReferrals=db.patientReferrals||[];db.professionalEvents=db.professionalEvents||[];db.journeyPurposeRecords=db.journeyPurposeRecords||[];db.familyJourneyRecords=db.familyJourneyRecords||[];db.settings={...defaultSettings,...(db.settings||{})};db.reflections=db.reflections||[];db.ideas=db.ideas||[];db.tasks=db.tasks.map(t=>({estimatedMinutes:0,nextAction:'',waitingFor:'',waitingContact:'',repeat:'None',focus:false,progress:t.status==='Done'?100:0,...t}));
...
const normalizeDb=x=>{
  x=x||{};
  x.tasks=x.tasks||[];x.study=x.study||[];x.wellness=x.wellness||[];x.travel=x.travel||[];x.ideas=x.ideas||[];x.travelStays=x.travelStays||[];x.travelDocs=x.travelDocs||[];x.emergencyContacts=x.emergencyContacts||[];x.travelFinance=x.travelFinance||[];x.healthProviders=x.healthProviders||[];x.healthContacts=x.healthContacts||[];x.patientReferrals=x.patientReferrals||[];x.professionalEvents=x.professionalEvents||[];x.journeyPurposeRecords=x.journeyPurposeRecords||[];x.familyJourneyRecords=x.familyJourneyRecords||[];
...
const TRAVEL_PACKING_MASTER=[
['Medical & First Aid','Regular personal prescription medicines','core,medical,all'],['Medical & First Aid','Copy/photo of prescriptions + generic medicine list','core,medical,documents,all'],['Medical & First Aid','Medical summary: conditions, allergies, blood group, emergency contact','core,medical,documents,all'],['Medical & First Aid','Paracetamol / acetaminophen — pain or fever, if suitable','core,medical,common'],['Medical & First Aid','Ibuprofen / NSAID option — pain/inflammation, only if suitable','medical,common'],['Medical & First Aid','Antacid / acid-blocker used personally for reflux/gastritis','medical,common'],['Medical & First Aid','Antiemetic / motion-sickness medicine used or prescribed personally','medical,train,bus,car,flight'],['Medical & First Aid','ORS / oral rehydration electrolyte sachets','core,medical,common'],['Medical & First Aid','Antidiarrheal for appropriate non-bloody diarrhea (e.g. loperamide), if suitable','medical,common'],['Medical & First Aid','Antihistamine for allergy, if suitable','medical,common'],['Medical & First Aid','Cough lozenges / throat soothing tablets','medical,common'],['Medical & First Aid','Usual cough/cold medicine if personally appropriate','medical,common'],['Medical & First Aid','Nasal saline / usual decongestant if needed','medical,flight,common'],['Medical & First Aid','Mild laxative if personally prone to constipation','medical,long'],['Medical & First Aid','Clinician-defined antibiotic — prescription-only; carry only if specifically indicated','medical,prescription'],['Medical & First Aid','Clinician-defined muscle relaxant — prescription-only; carry only if specifically indicated','medical,prescription'],['Medical & First Aid','Usual menstrual pain medicine, if personally suitable','medical,female'],['Medical & First Aid','Topical pain-relief balm / oil / spray used personally','medical,common'],['Medical & First Aid','Digital thermometer','medical,core'],['Medical & First Aid','Adhesive bandages / Band-Aids assorted sizes','medical,core'],['Medical & First Aid','Sterile gauze + non-stick dressing','medical,core'],['Medical & First Aid','Medical adhesive tape','medical,core'],['Medical & First Aid','Elastic / crepe bandage for sprain support','medical,core'],['Medical & First Aid','Antiseptic wipes / solution','medical,core'],['Medical & First Aid','Small scissors / nail cutter — pack according to transport security rules','medical'],['Medical & First Aid','Tweezers','medical'],['Medical & First Aid','Hand sanitizer','core,hygiene,medical'],['Medical & First Aid','Face masks','medical,core'],['Medical & First Aid','Sunscreen','medical,outdoor'],['Medical & First Aid','Insect repellent','medical,outdoor'],['Medical & First Aid','Glucose / sugar sachet for known hypoglycemia risk','medical,patient'],['Medical & First Aid','Disease-specific device/supplies (inhaler, glucometer, insulin etc.)','medical,patient'],['Medical & First Aid','Spare spectacles / contact lenses + solution','medical,core'],['Medical & First Aid','Heat/cold patch or hot-water-bag alternative if needed','medical,female,joint'],
['Female / Menstrual Care','Sanitary pads / tampons / menstrual cup as preferred','female,core'],['Female / Menstrual Care','Disposable bags / hygiene pouches','female'],['Female / Menstrual Care','Spare underwear','female,core'],['Female / Menstrual Care','Period tracking / expected dates checked before trip','female,readiness'],['Female / Menstrual Care','Personal intimate hygiene products used routinely','female'],
['Personal Hygiene & Grooming','Toothbrush','core,hygiene'],['Personal Hygiene & Grooming','Toothpaste','core,hygiene'],['Personal Hygiene & Grooming','Tongue cleaner / floss','hygiene'],['Personal Hygiene & Grooming','Bath soap / body wash','core,hygiene'],['Personal Hygiene & Grooming','Shampoo','hygiene,long'],['Personal Hygiene & Grooming','Face wash','hygiene'],['Personal Hygiene & Grooming','Hand wash / small soap','hygiene'],['Personal Hygiene & Grooming','Hair oil / comb','hygiene'],['Personal Hygiene & Grooming','Moisturizer / cream','hygiene'],['Personal Hygiene & Grooming','Deodorant / perfume','hygiene'],['Personal Hygiene & Grooming','Talcum / body powder','hygiene'],['Personal Hygiene & Grooming','Shaving kit / razor / trimmer','hygiene,male,long'],['Personal Hygiene & Grooming','Nail cutter','hygiene,long'],['Personal Hygiene & Grooming','Tissues / wet wipes','core,hygiene'],['Personal Hygiene & Grooming','Toilet paper / pocket tissues','hygiene'],['Personal Hygiene & Grooming','Small mirror','hygiene'],['Personal Hygiene & Grooming','Laundry soap / detergent sachet','hygiene,long'],
['Electronics & Connectivity','Primary mobile phone','core,electronics'],['Electronics & Connectivity','Mobile charger + cable','core,electronics'],['Electronics & Connectivity','Power bank — keep in cabin/hand baggage when flying','core,electronics,flight'],['Electronics & Connectivity','Earphones / Bluetooth headset','electronics'],['Electronics & Connectivity','Laptop / tablet if needed','electronics,professional'],['Electronics & Connectivity','Laptop / tablet charger','electronics,professional'],['Electronics & Connectivity','Smartwatch / fitness tracker charger','electronics'],['Electronics & Connectivity','Multi-plug / extension board','electronics,long'],['Electronics & Connectivity','Travel adapter if destination needs it','electronics,international'],['Electronics & Connectivity','USB drive / external SSD if needed','electronics,professional'],['Electronics & Connectivity','Camera + charger / memory card','electronics,tour'],['Electronics & Connectivity','Torch / headlamp','electronics,outdoor'],['Electronics & Connectivity','Mobile data + voice recharge ACTIVE before departure','core,readiness,electronics'],['Electronics & Connectivity','Offline maps / tickets / hotel details downloaded','core,readiness,electronics'],['Electronics & Connectivity','Important phone numbers available offline/on paper','core,readiness,electronics'],['Electronics & Connectivity','Cloud sync / backup completed before departure','readiness,electronics'],
['Clothing','Underwear — trip days + spare','core,clothing'],['Clothing','Socks — trip days + spare','clothing'],['Clothing','Daily shirts / T-shirts / kurta','core,clothing'],['Clothing','Trousers / jeans / pajama','core,clothing'],['Clothing','Nightwear / sleepwear','core,clothing'],['Clothing','Formal dress for seminar/interview/exam/official work','clothing,professional'],['Clothing','Traditional / festival / marriage dress','clothing,family'],['Clothing','Inner / thermal wear for cold climate','clothing,cold'],['Clothing','Sweater / jacket / shawl','clothing,cold'],['Clothing','Raincoat / poncho','clothing,rain'],['Clothing','Umbrella','clothing,rain'],['Clothing','Cap / hat','clothing,outdoor'],['Clothing','Handkerchiefs','clothing'],['Clothing','Extra set in hand bag for delay/emergency','core,clothing,long'],
['Footwear & Accessories','Comfortable walking shoes','core,footwear'],['Footwear & Accessories','Slippers / sandals','core,footwear'],['Footwear & Accessories','Formal shoes if needed','footwear,professional'],['Footwear & Accessories','Shoe polish / brush','footwear,professional'],['Footwear & Accessories','Belt','clothing,professional'],['Footwear & Accessories','Sunglasses','outdoor'],['Footwear & Accessories','Watch','accessory'],['Footwear & Accessories','Reusable shoe bag / plastic cover','footwear'],
['Food & Water','Drinking water bottle','core,food'],['Food & Water','Fresh home-cooked food for first journey segment','food,train,bus,car'],['Food & Water','Dry snacks / namkeen / roasted items','food,core'],['Food & Water','Fruits suitable for journey','food'],['Food & Water','Biscuits / crackers / dry snacks','food'],['Food & Water','Pickle / chutney travel pack','food,long'],['Food & Water','Dry fruits / nuts','food'],['Food & Water','Fasting / vrata food items if applicable','food,fasting'],['Food & Water','Tea/coffee sachets if needed','food,long'],['Food & Water','Spoon / small bowl / reusable cutlery','food,long'],['Food & Water','Napkins / food wipes','food'],['Food & Water','ORS / electrolyte drink backup','food,medical'],['Food & Water','Destination food plan checked: veg / Jain / no onion-garlic etc.','core,readiness,food'],
['Documents & Identity','Aadhaar / accepted photo ID','core,documents'],['Documents & Identity','Masked Aadhaar / digital copy in DigiLocker where appropriate','documents,backup'],['Documents & Identity','PAN card if required','documents'],['Documents & Identity','Driving licence if driving / renting vehicle','documents,car,bike'],['Documents & Identity','Passport + visa for international travel','documents,international'],['Documents & Identity','Travel tickets / PNR / boarding pass','core,documents'],['Documents & Identity','Hotel / guest house / ashram booking proof','documents,core'],['Documents & Identity','Travel insurance / health insurance details when applicable','documents,medical'],['Documents & Identity','Passport-size photos','documents,professional'],['Documents & Identity','Professional registration card / medical council ID','documents,professional'],['Documents & Identity','Visiting cards','documents,professional'],['Documents & Identity','Marksheets / degree certificates if required','documents,professional'],['Documents & Identity','Experience certificates / joining letters','documents,professional'],['Documents & Identity','Admit card / exam call letter','documents,professional'],['Documents & Identity','Registration form / payment receipt','documents,professional'],['Documents & Identity','Workshop / seminar brochure / delegate confirmation','documents,professional'],['Documents & Identity','Paper/poster/presentation files + backup','documents,professional'],['Documents & Identity','Printed emergency contacts + stay address','documents,core'],['Documents & Identity','Two backup copies of critical travel documents','documents,backup'],
['Money & Payments','Wallet','core,finance'],['Money & Payments','Cash reserve in small denominations','core,finance'],['Money & Payments','Primary debit / credit card','finance'],['Money & Payments','Secondary payment card / backup account','finance,backup'],['Money & Payments','UPI active + PIN remembered securely','core,finance,readiness'],['Money & Payments','Bank balance / travel budget checked','finance,readiness'],['Money & Payments','Emergency money kept separately','finance,backup'],['Money & Payments','Important bank/card helplines saved','finance,backup'],
['Bags, Bedding & Protection','Main travel bag / suitcase','core,bags'],['Bags, Bedding & Protection','Small backpack / day bag','core,bags'],['Bags, Bedding & Protection','Small sling / waist pouch','bags'],['Bags, Bedding & Protection','Waterproof bag / rain cover','bags,rain'],['Bags, Bedding & Protection','Zip-lock / document waterproof pouches','bags,core'],['Bags, Bedding & Protection','Bedsheet','bedding,train,long'],['Bags, Bedding & Protection','Towel','core,bedding'],['Bags, Bedding & Protection','Light blanket / shawl','bedding,train,bus,cold'],['Bags, Bedding & Protection','Thick cloth / mat for sleeping or sitting','bedding,train,bus'],['Bags, Bedding & Protection','Travel pillow / neck pillow','bedding,train,bus,flight'],['Bags, Bedding & Protection','Eye mask / ear plugs','bedding,train,bus,flight'],['Bags, Bedding & Protection','Bag lock / luggage tag','core,bags'],['Bags, Bedding & Protection','Laundry / dirty-clothes bag','bags,long'],
['Stationery & Work','Pens (2+)','core,stationery,professional'],['Stationery & Work','Small diary / notebook','stationery,professional'],['Stationery & Work','Pencil / marker / highlighter','stationery,professional'],['Stationery & Work','Glue / tape / stapler if needed','stationery,professional'],['Stationery & Work','Printed itinerary / address / notes','stationery,core'],['Stationery & Work','Presentation clicker / pointer if lecturing','stationery,professional'],['Stationery & Work','Name badge / institution ID','stationery,professional'],
['Gifts, Puja & Special Carry','Mithai / sweets for host or family','family,gift'],['Gifts, Puja & Special Carry','Gift / shagun / envelope','family,gift'],['Gifts, Puja & Special Carry','Temple offering / puja items if planned','family,pilgrimage'],['Gifts, Puja & Special Carry','Books / prasadam / special requested item','family,gift'],['Gifts, Puja & Special Carry','Items to deliver to a specific person','family,gift'],
['Home Shutdown Before Leaving','Main door / room locks checked','core,home,readiness'],['Home Shutdown Before Leaving','Windows / balcony / shutters secured','home,readiness'],['Home Shutdown Before Leaving','Gas cylinder / stove OFF','core,home,readiness'],['Home Shutdown Before Leaving','Water taps checked OFF','core,home,readiness'],['Home Shutdown Before Leaving','Water motor / pump OFF or scheduled safely','home,readiness'],['Home Shutdown Before Leaving','Iron / heater / geyser / induction OFF','core,home,readiness'],['Home Shutdown Before Leaving','Unnecessary electrical appliances unplugged','home,readiness'],['Home Shutdown Before Leaving','Refrigerator decision checked for long absence','home,long'],['Home Shutdown Before Leaving','Almirah / valuables locked','core,home,readiness'],['Home Shutdown Before Leaving','Keys placed safely / spare key handed to trusted person','core,home,readiness'],['Home Shutdown Before Leaving','CCTV / alarm / Wi-Fi required for security checked','home,readiness'],['Home Shutdown Before Leaving','Electricity / water / gas / internet bills checked','home,readiness'],['Home Shutdown Before Leaving','Milk delivery paused','home,long'],['Home Shutdown Before Leaving','Newspaper delivery paused','home,long'],['Home Shutdown Before Leaving','Water-can / domestic delivery paused','home,long'],['Home Shutdown Before Leaving','Garbage removed / perishables checked','home,readiness'],['Home Shutdown Before Leaving','Plants / pets / dependent-care arrangement made','home,long'],['Home Shutdown Before Leaving','Vehicle parked safely / fuel / cover / key managed','home,long'],['Home Shutdown Before Leaving','Trusted family/neighbour informed when appropriate','home,long'],
['Departure & Last-Mile Readiness','Station/airport/bus pickup cab or auto pre-booked','core,readiness'],['Departure & Last-Mile Readiness','Leave-home time calculated with safety buffer','core,readiness'],['Departure & Last-Mile Readiness','Ticket / PNR / platform / terminal rechecked','core,readiness'],['Departure & Last-Mile Readiness','Live train/flight/bus status checked before leaving','core,readiness'],['Departure & Last-Mile Readiness','Boarding ID kept in quick-access pouch','core,readiness'],['Departure & Last-Mile Readiness','Phone battery ≥80% before departure','core,readiness,electronics'],['Departure & Last-Mile Readiness','Power bank charged','core,readiness,electronics'],['Departure & Last-Mile Readiness','All chargers collected after final charging','core,readiness,electronics'],['Departure & Last-Mile Readiness','Cash + cards + UPI tested / available','core,readiness,finance'],['Departure & Last-Mile Readiness','Water + first meal/snack ready','core,readiness,food'],['Departure & Last-Mile Readiness','Medicines for journey kept in hand/day bag, not buried in main luggage','core,readiness,medical'],['Departure & Last-Mile Readiness','Destination host / hotel / hospital informed of ETA if needed','readiness'],['Departure & Last-Mile Readiness','Return / onward booking and backup route reviewed','readiness,long']
].map((x,i)=>({id:'pk_'+i,category:x[0],label:x[1],tags:x[2].split(',')}));
const PACKING_CATEGORIES=[...new Set(TRAVEL_PACKING_MASTER.map(x=>x.category))];
function normalizePackState(t={}){return {selected:new Set(Array.isArray(t.packingSelected)?t.packingSelected:[]),packed:new Set(Array.isArray(t.packingPacked)?t.packingPacked:[]),custom:Array.isArray(t.packingCustom)?t.packingCustom:[],notes:t.packingNotes||''}}
let packState={selected:new Set(),packed:new Set(),custom:[],notes:''};
function initTravelPacking(t={}){
  packState=normalizePackState(t); fillOptions($('#tr_pack_category'),PACKING_CATEGORIES,true,'All categories'); fillOptions($('#tr_pack_custom_category'),PACKING_CATEGORIES);
  $('#tr_pack_profile').value=t.packingProfile||'Self / Adult';$('#tr_pack_duration').value=t.packingDuration||'auto';$('#tr_pack_mode').value=t.packingMode||'Auto from trip';$('#tr_pack_notes').value=t.packingNotes||'';
  $('#tr_pack_search').oninput=renderPackingMaster;$('#tr_pack_category').onchange=renderPackingMaster;$('#tr_pack_profile').onchange=renderPackingMaster;
  $('#tr_pack_suggest').onclick=smartSelectPacking;$('#tr_pack_essentials').onclick=()=>{TRAVEL_PACKING_MASTER.filter(x=>x.tags.includes('core')).forEach(x=>packState.selected.add(x.id));renderPackingMaster();renderPackingFinal()};
  $('#tr_pack_allpacked').onclick=()=>{packState.selected.forEach(id=>packState.packed.add(id));packState.custom.forEach(x=>{if(x.selected)x.packed=true});renderPackingMaster();renderPackingCustom();renderPackingFinal()};
  $('#tr_pack_clear').onclick=()=>{if(confirm('Clear current trip packing selection?')){packState.selected.clear();packState.packed.clear();packState.custom.forEach(x=>{x.selected=false;x.packed=false});renderPackingMaster();renderPackingCustom();renderPackingFinal()}};
  $('#tr_pack_custom_add').onclick=addPackingCustom;$('#tr_pack_copy').onclick=()=>copyText(buildPackingSummary(),'Final packing list copied.');$('#tr_pack_share').onclick=()=>shareText(buildPackingSummary(),'Travel Packing & Readiness');$('#tr_pack_print').onclick=()=>printTextCard('Travel Packing & Departure Readiness',buildPackingSummary());
  renderPackingMaster();renderPackingCustom();renderPackingFinal();
}
function packTripDays(){let a=$('#tr_start')?.value||'',b=$('#tr_return')?.value||'';return tripDays(a,b).days||1}
function smartSelectPacking(){
  const prof=$('#tr_pack_profile').value,mode=$('#tr_pack_mode').value==='Auto from trip'?($('#tr_mode')?.value||'Train'):$('#tr_pack_mode').value;
  const dv=$('#tr_pack_duration').value,days=dv==='auto'?packTripDays():(+dv||1);const tags=new Set(['core','readiness']);
  if(days>=3)tags.add('long');if(/Train/i.test(mode))tags.add('train');if(/Bus/i.test(mode))tags.add('bus');if(/Car/i.test(mode))tags.add('car');if(/Bike/i.test(mode))tags.add('bike');if(/Flight/i.test(mode))tags.add('flight');
  if(/Female|Couple|Family/i.test(prof))tags.add('female');if(/Patient|Senior/i.test(prof)){tags.add('medical');tags.add('patient')}if(/Professional|Exam/i.test(prof))tags.add('professional');if(/Family|Couple/i.test(prof)){tags.add('family');tags.add('gift')}
  TRAVEL_PACKING_MASTER.forEach(x=>{if(x.tags.some(t=>tags.has(t)))packState.selected.add(x.id)});renderPackingMaster();renderPackingFinal();
}
function renderPackingMaster(){const box=$('#tr_pack_master');if(!box)return;const q=($('#tr_pack_search').value||'').toLowerCase(),cat=$('#tr_pack_category').value||'';const prof=$('#tr_pack_profile').value||'';let items=TRAVEL_PACKING_MASTER.filter(x=>(!cat||x.category===cat)&&(!q||(`${x.label} ${x.category}`).toLowerCase().includes(q)));
 const groups=PACKING_CATEGORIES.map(c=>[c,items.filter(x=>x.category===c)]).filter(x=>x[1].length);box.innerHTML=groups.map(([c,arr])=>`<details class="packing-group" ${cat===c||q?'open':''}><summary><span>${esc(c)}</span><b>${arr.filter(x=>packState.selected.has(x.id)).length}/${arr.length}</b></summary><div class="packing-items">${arr.map(x=>`<div class="packing-item ${packState.selected.has(x.id)?'selected':''} ${packState.packed.has(x.id)?'packed':''}"><label class="pack-include"><input type="checkbox" data-pack-select="${x.id}" ${packState.selected.has(x.id)?'checked':''}> Include</label><div class="pack-label">${esc(x.label)}</div><label class="pack-done"><input type="checkbox" data-pack-done="${x.id}" ${packState.packed.has(x.id)?'checked':''} ${packState.selected.has(x.id)?'':'disabled'}> Packed/Done</label></div>`).join('')}</div></details>`).join('');
 $$('[data-pack-select]').forEach(e=>e.onchange=()=>{let id=e.dataset.packSelect;e.checked?packState.selected.add(id):(packState.selected.delete(id),packState.packed.delete(id));renderPackingMaster();renderPackingFinal()});$$('[data-pack-done]').forEach(e=>e.onchange=()=>{let id=e.dataset.packDone;e.checked?packState.packed.add(id):packState.packed.delete(id);renderPackingMaster();renderPackingFinal()});updatePackingStats()}
function addPackingCustom(){let text=$('#tr_pack_custom_text').value.trim();if(!text)return;packState.custom.push({id:'custom_'+uid(),category:$('#tr_pack_custom_category').value||'Other',label:text,selected:true,packed:false});$('#tr_pack_custom_text').value='';renderPackingCustom();renderPackingFinal()}
function renderPackingCustom(){const box=$('#tr_pack_custom_list');if(!box)return;box.innerHTML=packState.custom.map(x=>`<div class="packing-custom-item"><label><input type="checkbox" data-custom-select="${x.id}" ${x.selected?'checked':''}> Include</label><span><b>${esc(x.category)}</b> • ${esc(x.label)}</span><label><input type="checkbox" data-custom-done="${x.id}" ${x.packed?'checked':''} ${x.selected?'':'disabled'}> Packed</label><button class="ghost danger-lite" data-custom-del="${x.id}">×</button></div>`).join('')||'<p class="muted">No custom items added for this trip.</p>';$$('[data-custom-select]').forEach(e=>e.onchange=()=>{let x=packState.custom.find(z=>z.id===e.dataset.customSelect);if(x){x.selected=e.checked;if(!e.checked)x.packed=false}renderPackingCustom();renderPackingFinal()});$$('[data-custom-done]').forEach(e=>e.onchange=()=>{let x=packState.custom.find(z=>z.id===e.dataset.customDone);if(x)x.packed=e.checked;renderPackingCustom();renderPackingFinal()});$$('[data-custom-del]').forEach(e=>e.onclick=()=>{packState.custom=packState.custom.filter(x=>x.id!==e.dataset.customDel);renderPackingCustom();renderPackingFinal()})}
function selectedPackingRows(){let rows=TRAVEL_PACKING_MASTER.filter(x=>packState.selected.has(x.id)).map(x=>({...x,packed:packState.packed.has(x.id)}));rows.push(...packState.custom.filter(x=>x.selected).map(x=>({...x,packed:!!x.packed})));return rows}
function renderPackingFinal(){const box=$('#tr_pack_final');if(!box)return;const rows=selectedPackingRows();const by=PACKING_CATEGORIES.concat(['Other']).map(c=>[c,rows.filter(x=>x.category===c)]).filter(x=>x[1].length);box.innerHTML=by.map(([c,a])=>`<div class="packing-final-group"><h4>${esc(c)} <span>${a.filter(x=>x.packed).length}/${a.length} done</span></h4>${a.map(x=>`<div class="packing-final-line ${x.packed?'done':'pending'}"><span>${x.packed?'☑':'☐'}</span>${esc(x.label)}</div>`).join('')}</div>`).join('')||'<p class="muted">Select items from the master checklist to create this trip’s final list.</p>';updatePackingStats()}
function updatePackingStats(){const rows=selectedPackingRows(),done=rows.filter(x=>x.packed).length,pending=rows.length-done;const e=$('#tr_pack_stats');if(e)e.innerHTML=`Selected <b>${rows.length}</b> • Packed/Done <b>${done}</b> • Pending <b>${pending}</b>`}
function buildPackingSummary(){const rows=selectedPackingRows();let out=`SAO WORKPLACE — TRAVEL PACKING & DEPARTURE READINESS\nTrip: ${$('#tr_title')?.value||'-'}\nRoute: ${$('#tr_origin')?.value||'-'} → ${$('#tr_place')?.value||'-'}\nDates: ${$('#tr_start')?.value||'-'} → ${$('#tr_return')?.value||'-'}\nTraveler profile: ${$('#tr_pack_profile')?.value||'-'}\n\n`;
 PACKING_CATEGORIES.concat(['Other']).forEach(c=>{const a=rows.filter(x=>x.category===c);if(a.length){out+=`${c.toUpperCase()}\n`;a.forEach(x=>out+=`${x.packed?'☑':'☐'} ${x.label}\n`);out+='\n'}});out+=`PACKED/DONE: ${rows.filter(x=>x.packed).length}/${rows.length}\nPENDING: ${rows.filter(x=>!x.packed).length}\n\nNOTES\n${$('#tr_pack_notes')?.value||'-'}\n\nMedical reminder: packing checklist only; use medicines according to the individual traveler's clinician-approved plan and applicable transport/destination rules.`;return out}
...
function travelPlanTemplate(t={}){
  ...
  <div class="travel-v6-command">
    ...
    <div class="travel-v6-shortcuts">
      ...
      <button type="button" id="tr_jump_pack">🎒 Packing List<small>Carry & departure</small></button>
  ...
  <div class="travel-section packing-intelligence" id="travelPackingIntelligence">
    <div class="travel-section-title"><span>9</span><div><b>🎒 Universal Travel Packing & Departure Readiness</b><small>Master checklist → smart suggestions → trip-specific selection → packed/done verification → final PDF/share list.</small></div></div>
    <div class="packing-safety-note"><b>Medical safety:</b> this is a packing/reminder system, not a prescription. Regular medicines should match the traveler’s own clinician-approved plan. Prescription-only medicines (including antibiotics or muscle relaxants) should be carried only when specifically prescribed/appropriate. Check destination and transport rules before travel.</div>
    <div class="packing-command-grid">
      <label>Traveler profile<select id="tr_pack_profile"><option>Self / Adult</option><option>Male Adult</option><option>Female Adult</option><option>Couple / Family</option><option>Child / Infant</option><option>Senior Citizen</option><option>Patient / Medical Travel</option><option>Professional / Exam Travel</option></select></label>
      <label>Trip duration<select id="tr_pack_duration"><option value="auto">Auto from trip dates</option><option value="1">1 day</option><option value="3">2–3 days</option><option value="7">4–7 days</option><option value="14">8–14 days</option><option value="30">15+ days</option></select></label>
      <label>Travel mode<select id="tr_pack_mode"><option>Auto from trip</option><option>Train</option><option>Bus</option><option>Car</option><option>Bike</option><option>Flight</option><option>Mixed / Multi-mode</option></select></label>
      <label>Checklist filter<select id="tr_pack_category"><option value="">All categories</option></select></label>
    </div>
    <div class="packing-actions">
      <button type="button" id="tr_pack_suggest" class="travel-ai-btn">✨ Smart Select for This Trip</button>
      <button type="button" id="tr_pack_essentials" class="ghost">✓ Select Core Essentials</button>
      <button type="button" id="tr_pack_allpacked" class="ghost">☑ Mark Selected as Packed</button>
      <button type="button" id="tr_pack_clear" class="ghost">Clear Selection</button>
    </div>
    <div class="packing-filter-row"><input id="tr_pack_search" placeholder="Search medicine, charger, Aadhaar, towel, gift, gas, key…"><div id="tr_pack_stats" class="packing-stats"></div></div>
    <div id="tr_pack_master" class="packing-master"></div>
    <div class="packing-custom card-soft">
      <div><b>➕ Other / Custom item</b><small>Add anything unique to this journey that is not in the universal master list.</small></div>
      <div class="packing-custom-row"><select id="tr_pack_custom_category"></select><input id="tr_pack_custom_text" placeholder="e.g. मिठाई for family / special instrument / temple offering"><button type="button" id="tr_pack_custom_add">Add</button></div>
      <div id="tr_pack_custom_list"></div>
    </div>
    <div class="packing-final card-soft">
      <div class="train-subhead"><div><b>📋 Final Trip-Specific Carry & Departure List</b><small>Only selected items appear here. Packed/Done and Pending remain separate.</small></div></div>
      <div id="tr_pack_final"></div>
      <label>Trip-specific packing notes<textarea id="tr_pack_notes" placeholder="Bag split, who carries what, fragile items, special medicines, temple/gift items, last-minute reminders…"></textarea></label>
      <div class="packing-export-actions"><button type="button" id="tr_pack_copy">📋 Copy Final List</button><button type="button" id="tr_pack_share" class="ghost">↗ Share / WhatsApp</button><button type="button" id="tr_pack_print" class="ghost">🖨 Print / Save PDF</button></div>
    </div>
  </div>

  <div class="travel-section emergency-pack" id="travelEmergencyPack">
    <div class="travel-section-title"><span>10</span><div><b>🆘 Emergency & Recovery Pack</b><small>What you need if phone, wallet, documents or connectivity are lost.</small></div></div>
...
function editTravel(id=''){
  ...
  $('#tr_jump_pack').onclick=()=>$('#travelPackingIntelligence')?.scrollIntoView({behavior:'smooth'});
  ...
  renderStayBookings(); renderTravelDocs(); renderEmergencyContacts(); renderTravelFinance(); initTravelPacking(t||{}); updateTravelReadiness();
  ...
  $('#saveTravel').onclick=()=>{
    ...
    const x={
      ...
      returnRef:$('#tr_return_ref').value,liveNote:$('#tr_live_note').value,rulesNote:$('#tr_rules_note').value,dayFlowHtml:$('#tr_day_flow').innerHTML,notes:$('#tr_notes').value,packingSelected:[...packState.selected],packingPacked:[...packState.packed],packingCustom:packState.custom,packingNotes:$('#tr_pack_notes')?.value||'',packingProfile:$('#tr_pack_profile')?.value||'Self / Adult',packingDuration:$('#tr_pack_duration')?.value||'auto',packingMode:$('#tr_pack_mode')?.value||'Auto from trip',updatedAt:new Date().toISOString(),createdAt:t?.createdAt||new Date().toISOString()
    };
...
function updateTravelReadiness(){
  ...
  if((db.travelStays||[]).length)score+=10;if((db.travelDocs||[]).length>=2)score+=10;if((db.emergencyContacts||[]).length>=2)score+=10;if((db.travelFinance||[]).length)score+=10;const pr=selectedPackingRows?.()||[];if(pr.length>=10)score+=5;if(pr.length&&pr.every(x=>x.packed))score+=5;
...
${document.querySelector('#tr_pack_final')?buildPackingSummary():''}`;
}
...
const JP_FIELDS="category formality title orgtype org department city venue address map startdate starttime enddate endtime reporting tithi contact phone email url status priority with other reason regno candidateid callletter fee paymode payref dress docsrequired materials topic audience duration checklist schedule route stay backup outcome certificate honorarium reimbursement resultnotes learning followup".split(" ");
const FP_FIELDS="category title host relation city venue address map startdate starttime enddate endtime tithi priority status other reason with contact phone staytype stay food gift items healthvisit hospital specialty apptdate appttime patient diagnosis estcost healthnotes emergency outcome spend good improve followup".split(" ");
function gv(p,k){return $('#'+p+'_'+k)?.value||''} function sv(p,k,v){let e=$('#'+p+'_'+k);if(e)e.value=v??''}
function initJourneyPurposeModes(){const t=$('#travelPrimary'),pro=$('#proEventPanel'),jp=$('#purposeEventPanel'),fp=$('#familyPurposePanel');const all=()=>{[t,pro,jp,fp].forEach(x=>{if(x)x.hidden=true});[$('#proTravelMode'),$('#proEventMode'),$('#examModeBtn'),$('#familyModeBtn')].forEach(b=>b?.classList.remove('active'))};$('#proTravelMode').onclick=()=>{all();if(t)t.hidden=false;$('#proTravelMode').classList.add('active')};$('#proEventMode').onclick=()=>{all();if(pro)pro.hidden=false;$('#proEventMode').classList.add('active');if(typeof peDraw==='function')peDraw();if(typeof peCount==='function')peCount()};$('#examModeBtn').onclick=()=>{all();jp.hidden=false;$('#examModeBtn').classList.add('active');jpDraw();jpCount()};$('#familyModeBtn').onclick=()=>{all();fp.hidden=false;$('#familyModeBtn').classList.add('active');fpDraw();fpCount()};$$('.purpose-tabs button[data-purpose-tab]').forEach(b=>b.onclick=()=>{$$('.purpose-tabs button[data-purpose-tab]').forEach(x=>x.classList.remove('active'));b.classList.add('active');$$('.purpose-pane').forEach(x=>x.hidden=true);$('#purpose_'+b.dataset.purposeTab).hidden=false});$$('.purpose-tabs button[data-family-tab]').forEach(b=>b.onclick=()=>{$$('.purpose-tabs button[data-family-tab]').forEach(x=>x.classList.remove('active'));b.classList.add('active');$$('.family-pane').forEach(x=>x.hidden=true);$('#family_'+b.dataset.familyTab).hidden=false});$('#jp_save').onclick=jpSave;$('#jp_clear').onclick=jpClear;$('#jp_share').onclick=()=>shareText(jpSummary(jpRecord()),'Purpose Journey');$('#jp_print').onclick=()=>printTextCard('Purpose Journey',jpSummary(jpRecord()));$('#jp_attach').onclick=jpAttach;$('#jp_search').oninput=jpDraw;$('#jp_filter').onchange=jpDraw;$('#fp_save').onclick=fpSave;$('#fp_clear').onclick=fpClear;$('#fp_share').onclick=()=>shareText(fpSummary(fpRecord()),'Special Journey');$('#fp_print').onclick=()=>printTextCard('Special Journey',fpSummary(fpRecord()));$('#fp_attach').onclick=fpAttach;$('#fp_search').oninput=fpDraw;$('#fp_filter').onchange=fpDraw;if(!gv('jp','startdate'))sv('jp','startdate',today());if(!gv('fp','startdate'))sv('fp','startdate',today())}
function jpRecord(){let x={id:gv('jp','id')||uid(),updatedAt:new Date().toISOString()};JP_FIELDS.forEach(k=>x[k]=gv('jp',k));let o=db.journeyPurposeRecords.find(z=>z.id===x.id);if(o?.documents)x.documents=o.documents;return x} function fpRecord(){let x={id:gv('fp','id')||uid(),updatedAt:new Date().toISOString()};FP_FIELDS.forEach(k=>x[k]=gv('fp',k));let o=db.familyJourneyRecords.find(z=>z.id===x.id);if(o?.documents)x.documents=o.documents;return x}
function jpSave(){let x=jpRecord();if(!x.title)return alert('Title / reason is required.');let i=db.journeyPurposeRecords.findIndex(z=>z.id===x.id);i>=0?db.journeyPurposeRecords[i]=x:db.journeyPurposeRecords.push(x);save();sv('jp','id',x.id);jpDraw();jpCount();alert('Purpose journey saved successfully.')} function fpSave(){let x=fpRecord();if(!x.title)return alert('Title / occasion is required.');let i=db.familyJourneyRecords.findIndex(z=>z.id===x.id);i>=0?db.familyJourneyRecords[i]=x:db.familyJourneyRecords.push(x);save();sv('fp','id',x.id);fpDraw();fpCount();alert('Special journey saved successfully.')}
function jpClear(){sv('jp','id','');JP_FIELDS.forEach(k=>{let e=$('#jp_'+k);if(!e)return;e.tagName==='SELECT'?e.selectedIndex=0:e.value=''});sv('jp','startdate',today());$('#jp_doclist').innerHTML=''} function fpClear(){sv('fp','id','');FP_FIELDS.forEach(k=>{let e=$('#fp_'+k);if(!e)return;e.tagName==='SELECT'?e.selectedIndex=0:e.value=''});sv('fp','startdate',today());$('#fp_doclist').innerHTML=''}
function jpEdit(id){let x=db.journeyPurposeRecords.find(z=>z.id===id);if(!x)return;$('#examModeBtn')?.click();sv('jp','id',x.id);JP_FIELDS.forEach(k=>sv('jp',k,x[k]));jpDocs(id)} function fpEdit(id){let x=db.familyJourneyRecords.find(z=>z.id===id);if(!x)return;$('#familyModeBtn')?.click();sv('fp','id',x.id);FP_FIELDS.forEach(k=>sv('fp',k,x[k]));fpDocs(id)}
function jpDelete(id){if(confirm('Delete this purpose journey?')){db.journeyPurposeRecords=db.journeyPurposeRecords.filter(x=>x.id!==id);save();jpDraw();jpCount()}} function fpDelete(id){if(confirm('Delete this special journey?')){db.familyJourneyRecords=db.familyJourneyRecords.filter(x=>x.id!==id);save();fpDraw();fpCount()}}
function jpCount(){if($('#purposeCount'))$('#purposeCount').textContent=db.journeyPurposeRecords.length} function fpCount(){if($('#fpCount'))$('#fpCount').textContent=db.familyJourneyRecords.length}
function jpSummary(x){return `${x.category||''} • ${x.formality||''}\n${x.title||'Untitled'}\nOrganization: ${x.org||'-'} • ${x.orgtype||'-'} • ${x.department||'-'}\nCity/Venue: ${x.city||'-'} • ${x.venue||'-'}\nDate: ${x.startdate||'-'} ${x.starttime||''} → ${x.enddate||'-'} ${x.endtime||''}\nReporting: ${x.reporting||'-'} • Status ${x.status||'-'} • Priority ${x.priority||'-'}\nContact: ${x.contact||'-'} ${x.phone||''}\nRegistration/Candidate: ${x.regno||'-'} • ${x.candidateid||'-'} • Call letter ${x.callletter||'-'}\nFees: ₹${x.fee||0} • ${x.paymode||'-'} ${x.payref||''}\nTopic/Audience: ${x.topic||'-'} • ${x.audience||'-'}\nRequirements: ${x.docsrequired||'-'} • ${x.materials||'-'}\nSchedule: ${x.schedule||'-'}\nRoute: ${x.route||'-'}\nStay/Food: ${x.stay||'-'}\nBackup: ${x.backup||'-'}\nOutcome: ${x.outcome||'-'} • Certificate ${x.certificate||'-'}\nHonorarium/TA-DA: ₹${x.honorarium||0} • Reimbursement ${x.reimbursement||'-'}\nLearning: ${x.learning||'-'}\nFollow-up: ${x.followup||'-'}`}
function fpSummary(x){return `${x.category||''}\n${x.title||'Untitled'}\nHost/Context: ${x.host||'-'} • ${x.relation||'-'}\nCity/Venue: ${x.city||'-'} • ${x.venue||'-'}\nDate: ${x.startdate||'-'} ${x.starttime||''} → ${x.enddate||'-'} ${x.endtime||''}\nTithi/Festival: ${x.tithi||'-'} • Priority ${x.priority||'-'} • Status ${x.status||'-'}\nWith: ${x.with||'-'} • Contact ${x.contact||'-'} ${x.phone||''}\nStay: ${x.staytype||'-'} • ${x.stay||'-'}\nFood: ${x.food||'-'} • Gift ₹${x.gift||0}\nHealth visit: ${x.healthvisit||'-'} • ${x.hospital||'-'} • ${x.specialty||'-'}\nAppointment: ${x.apptdate||'-'} ${x.appttime||''}\nPatient/Reason: ${x.patient||'-'} • ${x.diagnosis||'-'}\nHealth notes: ${x.healthnotes||'-'}\nEmergency plan: ${x.emergency||'-'}\nOutcome: ${x.outcome||'-'} • Spend ₹${x.spend||0}\nGood: ${x.good||'-'}\nImprove: ${x.improve||'-'}\nFollow-up: ${x.followup||'-'}`}
function jpDraw(){let b=$('#jp_archive');if(!b)return;let q=($('#jp_search')?.value||'').toLowerCase(),f=$('#jp_filter')?.value||'';let L=db.journeyPurposeRecords.filter(x=>(!q||`${x.title} ${x.category} ${x.city} ${x.org}`.toLowerCase().includes(q))&&(!f||x.category===f));b.innerHTML=L.map(x=>`<div class="professional-event-card"><div class="provider-head"><div><b>${esc(x.title||'Untitled')}</b><span>${esc(x.category||'')} • ${esc(x.city||'')} • ${esc(x.org||'')}</span></div><span class="ref-status">${esc(x.status||'')}</span></div><div class="provider-actions"><button class="ghost" onclick="app.jpEdit('${x.id}')">Open/Edit</button><button class="ghost" onclick="app.jpShare('${x.id}')">Share</button><button class="ghost" onclick="app.jpPrint('${x.id}')">PDF</button><button class="ghost danger-lite" onclick="app.jpDelete('${x.id}')">Delete</button></div></div>`).join('')||'<p class="muted">No purpose journey saved yet.</p>'}
function fpDraw(){let b=$('#fp_archive');if(!b)return;let q=($('#fp_search')?.value||'').toLowerCase(),f=$('#fp_filter')?.value||'';let L=db.familyJourneyRecords.filter(x=>(!q||`${x.title} ${x.category} ${x.city} ${x.host} ${x.hospital}`.toLowerCase().includes(q))&&(!f||x.category===f));b.innerHTML=L.map(x=>`<div class="professional-event-card"><div class="provider-head"><div><b>${esc(x.title||'Untitled')}</b><span>${esc(x.category||'')} • ${esc(x.city||'')} • ${esc(x.host||'')}</span></div><span class="ref-status">${esc(x.status||'')}</span></div><div class="provider-actions"><button class="ghost" onclick="app.fpEdit('${x.id}')">Open/Edit</button><button class="ghost" onclick="app.fpShare('${x.id}')">Share</button><button class="ghost" onclick="app.fpPrint('${x.id}')">PDF</button><button class="ghost danger-lite" onclick="app.fpDelete('${x.id}')">Delete</button></div></div>`).join('')||'<p class="muted">No special journey saved yet.</p>'}
function jpShare(id){let x=db.journeyPurposeRecords.find(z=>z.id===id);if(x)shareText(jpSummary(x),'Purpose Journey')} function jpPrint(id){let x=db.journeyPurposeRecords.find(z=>z.id===id);if(x)printTextCard('Purpose Journey',jpSummary(x))} function fpShare(id){let x=db.familyJourneyRecords.find(z=>z.id===id);if(x)shareText(fpSummary(x),'Special Journey')} function fpPrint(id){let x=db.familyJourneyRecords.find(z=>z.id===id);if(x)printTextCard('Special Journey',fpSummary(x))}
async function jpAttach(){let f=$('#jp_file')?.files?.[0];if(!f)return alert('Choose a document first.');let id=gv('jp','id');if(!id){let x=jpRecord();if(!x.title)return alert('Enter title first.');db.journeyPurposeRecords.push(x);save();id=x.id;sv('jp','id',id)}let fid='jp_'+uid();await putFile({id:fid,category:'Travel & Seminar',taskId:'',note:gv('jp','doctype'),name:f.name,type:f.type,size:f.size,date:today(),blob:f});let e=db.journeyPurposeRecords.find(x=>x.id===id);e.documents=e.documents||[];e.documents.push({id:uid(),fileId:fid,name:f.name,docType:gv('jp','doctype'),size:f.size});save();$('#jp_file').value='';$('#jp_docstatus').textContent='✅ Document saved locally.';jpDocs(id)}
async function fpAttach(){let f=$('#fp_file')?.files?.[0];if(!f)return alert('Choose a document first.');let id=gv('fp','id');if(!id){let x=fpRecord();if(!x.title)return alert('Enter title first.');db.familyJourneyRecords.push(x);save();id=x.id;sv('fp','id',id)}let fid='fp_'+uid();await putFile({id:fid,category:'Travel & Seminar',taskId:'',note:gv('fp','doctype'),name:f.name,type:f.type,size:f.size,date:today(),blob:f});let e=db.familyJourneyRecords.find(x=>x.id===id);e.documents=e.documents||[];e.documents.push({id:uid(),fileId:fid,name:f.name,docType:gv('fp','doctype'),size:f.size});save();$('#fp_file').value='';$('#fp_docstatus').textContent='✅ Document saved locally.';fpDocs(id)}
function jpDocs(id){let b=$('#jp_doclist');if(!b)return;let e=db.journeyPurposeRecords.find(x=>x.id===id),D=e?.documents||[];b.innerHTML=D.map(d=>`<div class="contact-card"><div><b>${esc(d.docType)}</b><span>${esc(d.name)}</span></div><div class="provider-actions"><button class="ghost" onclick="app.openFile('${d.fileId}')">Open</button><button class="ghost" onclick="app.downloadFile('${d.fileId}')">Download</button></div></div>`).join('')||'<p class="muted">No documents saved.</p>'} function fpDocs(id){let b=$('#fp_doclist');if(!b)return;let e=db.familyJourneyRecords.find(x=>x.id===id),D=e?.documents||[];b.innerHTML=D.map(d=>`<div class="contact-card"><div><b>${esc(d.docType)}</b><span>${esc(d.name)}</span></div><div class="provider-actions"><button class="ghost" onclick="app.openFile('${d.fileId}')">Open</button><button class="ghost" onclick="app.downloadFile('${d.fileId}')">Download</button></div></div>`).join('')||'<p class="muted">No documents saved.</p>'}
